...
from modules import zones_utils, od_utils, assignment_utils, validation_utils
...
def build_osm_safe_graph(G, city_name):
    ...
    if not nx.is_strongly_connected(G):
        ...
        print(f"Not strongly connected: {len(components)} components")
...
def run_city(CONFIG):
    ...
    if CONFIG.get("sensors_csv"):
        ...
        print("No sensors → validation skipped")
    ...
    hourly_OD = od_utils.generate_hourly_od_entropy(
        pop, D, CONFIG["hourly_profiles"], total_daily_trips=total_trips
    )
    ...
    od_utils.od_diagnostics(hourly_OD)
